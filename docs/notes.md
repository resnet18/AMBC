...
**判断**：官方 MiniRocket（tsai 路线）和 sktime MiniRocket 是两个不同的算法实现，收敛值差 20%（0.60 vs 0.72），这一点是时序特有的，跟之前的NLP区别很大，需要注意。
...
**实锤**：在自己的 tsai 实现中**故意注释掉 `np.transpose`**，完全复刻官方配置，结果准确率为 **0.5895 ± 0.0094**，与官方 59.8%、标答 A/B 的 0.5894 几乎完全一致。修正后（`c_in=23, seq_len=4096`）结果回到 **0.7318**，差距约 13 个百分点。
...
**教训**：交叉验证一致 ≠ 实现正确。审计必须包含代码逐行审查，不能因"多方一致"而跳过对实现细节的验证。tsai 的 `MiniRocketFeatures(c_in, seq_len)` 参数顺序必须显式核对。
...
### 9. 增加架构收录原则：别让主表变成超参扫描报告

因为看到很多社区复现者会自己做调参和消融，所以增加了这一条（避免benchmarj无限膨胀）：
**不收录的**：
- **纯调参版**：改学习率、epochs、batch size、特征数，这是炼丹，不是新架构
- **消融版**：比如 CNN+BiMamba 架构里只留下CNN，这是原技术报告自己该做的诊断，不作为主要结果
- **数据增强版**：用了评估协议3.1禁止的数据增强的， 改成标准协议，改不了的收录进Extended Track并标注

**可以收录的**：
- **同一算法的不同实现路径**：MiniRocket-sktime 和 MiniRocket-tsai 可以并列（并且标注）
- **结构性修改**：把 Mamba 换成 LSTM，或者加了完全不同的特征提取阶段——这算新架构，但得走审核

**判定原则**：如果两条目唯一的区别是"训练了多久"或者"提了多少特征"，弱的那个直接踢掉。AMBC 比的是**架构**，不是**预算**。
**教训**：基准维护者必须做恶人，不然主表会无限膨胀，最后谁都没法看。
...
- **InceptionTime**：PyTorch 移植顺利，官方配置（batch=128, 200 epochs）直接能跑，无需任何近似，已完跑 5-fold。
...
- 建统一虚拟环境和完整测试流水线（现在训练和推理还混在一块，环境也是各跑各的）
- 扩充指标：Precision、AUC-ROC、训练时间，让评估更贴近实际部署
- 修订协议（如果需要）
