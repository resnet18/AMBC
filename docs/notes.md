...
### 0. 动机升级轨迹（前置背景）
**原始计划**：仿照自己之前的一个独立研究项目模式，自己跑一堆常用模型作为基线，统一对比，画 Pareto frontier。目标是把 NGAFID 二分类做成有客观对比表的"小型 benchmark"。
...
NGAFID 预处理后的数据格式是 `(N, 4096, 23)`（时间步 × 传感器），但 tsai 要求 `(N, 23, 4096)`（传感器 × 时间步）。官方Colab代码直接取了 `train_X.shape[1]` 和 `train_X.shape[2]`，未进行转置，导致 `c_in=4096, seq_len=23`，也就是说，MiniRocket 的 1D 卷积核在 4096 个"伪通道"上、每个通道仅 23 个时间步运行，dilation 逻辑完全失效。

这一设计差异决定了传播路径：使用 sktime 的复现者（Lctong1021）因为库的自动容错而无意识幸存；使用 tsai 的复现者（官方、标答 A/B、SongX-1、ccDT2022）因为没有容错和照搬官方代码（未转置）而集体中招。另外，复现者m0_64843793的手搓版实现中显式加入了`np.transpose`，同样避开了陷阱，这说明该错误并非依赖问题，而是tsai的显式参数设计与官方代码的疏忽共同导致的系统性陷阱。若官方代码正确转置，tsai与sktime的结果应基本一致（tsai的NN分类头可能略高 1–2%），笔者的消融实验也证明了这一点。
...
原版基准（Yang et al., 2022）是 TensorFlow + Colab TPU v2-8。本地 RTX 5060 Ti（Blackwell / sm_120）完全跑不了TF（参见/archive里的归档版代码注释，这还不像mamba可以开wsl2，之前想着放wsl2但是也不兼容就是了），所以 InceptionTime 和 ConvMHSA 都重写了 PyTorch 版。
...
- **ConvMHSA**：和原论文基本一致，前述知乎复现者说的工程优化亲测也没什么用，可见别人说的也未必对，还是要自己验证
...
### 2026-7-9 第一阶段后记：不仅审计模型，也审计数据集

从头到尾，我一直有一个最大的疑问：**作者自己为什么没有把数据集做成benchmark？** 最开始我只是猜测作者认为数据不够典型，质量不够高，甚至往版权/合规方面猜；之后发现整个时序方面的测评生态都相对混乱（找到了一些相关的文献依据），也缺乏一个“统一接口”（就拿环境来说吧，NLP有固定的transformers库/huggingface这样的基础设施，而我在AMBC的制定过程中就已经踩了不少环境的坑了），在里面开荒很麻烦，作者可能不想踩这个浑水；现在做完回头看，这些或许都是次要的，**数据集本身的特性**可能才是根本原因。

#### 二分类任务的"高不成低不就"
我发现所有模型在二分类上都卡在 0.71-0.76 区间，这不是调参能解决的，是**任务定义本身的复杂程度上限**。`P(RUL>2days)` 这种粗糙的 yes/no 标签，把"慢性磨损"这种渐变问题硬压成二元判断，真正的故障信号被飞行员操作噪声淹没。论文作者自己比谁都清楚这一点，所以他们发了 ConvMHSA 的 IAAI-22 论文，公开了数据集和数据处理工具（ngafid2.0平台），转头就去搞别的了（笑）。似乎能看出来他们对benchmarking或许有兴趣`time_series_benchmarking` 仓库扔了个空壳就再没更新过。**行为模式说明一切：他们知道这数据不太对劲，但论文里必须写"for benchmarking"。**（或许多分类还有点用，但是类别不平衡，其实也不太好）

#### 什么样的数据集适合做成benchmark？
我的推测（参考CWRU数据集，虽然没成为benchmark，但是自发的都用这个比较）：信号-目标链路清晰、变量可控、信噪比足够、社区有刚需。而NGAFID只满足了"真实"和"大"，但b测评需要的任务合理性，物理可解释性，可比较性都不够，在基准测试方面确实不算合适。

#### 既然如此，AMBC 的意义还剩下什么？
当所有模型的准确率差异在统计噪声范围内时，效率方面的差异反而成了唯一的真实信号，这恰好印证了**可用性**的意义，二分类全部集中在0.7+这个发现本身，就是在说"性能已经到头了"。

#### 作为学校给的测试题的一份答卷
AMBC 展示的不是"我能跑出 0.76"或者"我做了个benchmark"，而是：
- 独立设计工程上完整的评估协议
- 做出扎实的审计工作（找到了MiniRocket官方实现的bug，识别出BiMamba使用滑动窗口导致的不公平对比）
- 在资源受限时做诚实近似（ConvMHSA工程优化）
- 判断"这个数据集在准确率维度上已经到头了（但效率维度还有价值）"
- 反思"如何做benchmark"这个元问题
- 与AI高效协作的同时保持自己的思想，而不是被AI带着走
