#!/usr/bin/env python3
"""
AMBC Experiment: MiniRocket + Statistical Features Fusion
===========================================================
Lctong1021's Stage 3 equivalent under AMBC Standard Protocol.
"""
...
def main(cfg):
    print("=" * 60)
    print("AMBC Experiment: MiniRocket + Statistical Features Fusion")
    print("=" * 60)
    ...
    for fold in range(5):
        ...
        # MiniRocket (sktime expects (N, C, T) = (N, 23, 4096))
        X_train_mr = np.transpose(X_train, (0, 2, 1))
        X_val_mr = np.transpose(X_val, (0, 2, 1))

        mr = MiniRocketMultivariate(num_kernels=cfg.num_kernels, n_jobs=-1)
        X_train_mr_feat = mr.fit_transform(X_train_mr).to_numpy(dtype=np.float32)
        X_val_mr_feat = mr.transform(X_val_mr).to_numpy(dtype=np.float32)

        # Fusion
        X_train_f = np.concatenate([X_train_stats, X_train_mr_feat], axis=1)
        X_val_f = np.concatenate([X_val_stats, X_val_mr_feat], axis=1)
        ...
        del X_train, X_val, X_train_stats, X_val_stats
        del X_train_mr, X_val_mr, X_train_mr_feat, X_val_mr_feat
        del X_train_f, X_val_f, scaler, clf
        gc.collect()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="experiments/fusion_minirocket_stats/outputs")
